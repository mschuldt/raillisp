...
(defun str-append (a b)
  ...
  (str-move! s a 0)
  (str-move! s b (string-length a)))

(defun str->list (s)
  (var end (1- (string-length s)))
  (var l nil)
  (dotimes (i (1+ end))
...
(defun list->str (char-list)
  (var len (list-length char-list))
  (var s (make-string len 0))
  (dotimes (i len)
...
(defun str-join (strings sep)
  (var strings-len 0)
  (var ret nil)
  (var sep-len (string-length sep))
  (let* ((len 0))
    (dolist (s strings)
      (set len (+ len (+ (string-length s) sep-len)))
      (set strings-len (1+ strings-len)))
    (set len (- len sep-len))
    (set ret (make-empty-string len)))
  (let* ((offset 0))
